...
@Client.on_message(filters.command(['directdl', 'direct', 'direct@lazy_leechv2bot', 'zipdirectdl', 'zipdirect', 'filedirectdl', 'filedirect']) & filters.chat(ALL_CHATS))
...
<b>╰─⌊ETA⌉</b> {calculate_eta(completed_length, total_length, start_time)}'''
    ...
    if torrent_info['status'] == 'error':
        ...
        await asyncio.gather(
            message.reply_text('I have cancelled your download'),
            reply.delete()
        )
    else:
...
@Client.on_message(filters.command(['list', 'list@lazy_leechv2bot']) & filters.chat(ALL_CHATS))
...
<code>{i['gid']}</code>\n\n'''
            ...
            text = futtext
    if not text:
        text = 'No downloads by you found'
    await message.reply_text(text, quote=quote)

@Client.on_message(filters.command(['cancel', 'cancel@lazy_leechv2bot']) & filters.chat(ALL_CHATS))
